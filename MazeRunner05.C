...
char restart(){                           //Function to execute the restart operation as in a regular game 
  char c;
  int i,j;
  clrscr();
  printf("\n\n\n\n\n\n\n\n\n\n\n\t\t         Are u sure u want to replay ???");
  printf("\n\t\t All the changes made in the game will be set to 0");
  printf("\n\t\t  Press r for restart or any other key to continue: ");
  c=getch();   //Users response
  if(c=='r') { maze[y][x]=maze[Y[0]][X[0]]=maze[Y[1]][X[1]]=maze[Y[2]][X[2]]=' ';   //Reset everything to zero
               main();                    //Start everything from first
			   }
  else{                                   //Display the maze and continue the game
     clrscr();
     for(i=0;i<23;i++){
	for(j=0;j<73;j++){
	   printf("%c",maze[i][j]);
...
void game_loop(){                                      //Function to start the real game:)
    ...
    while(maze[y][x]!='X'&&(flag!=2)){                 //To run the game till the player wins or loses
	  ...
	  while(flag==0){
	      ...
	      else if(ch=='r'){   ch=restart();              //Call the restart function if the user wants to restart the game
				  /*Exit the game if the person has restarted the game, as main() was called;
				  otherwise even after the player wins or loses, the old game gets resumed*/
				  if(ch=='r')  exit(0);                  //Exit the game as main() was called again
...
int main(){
   ...
   printf("\n\t       Moves: up=w  down=s  left=a  right=d  exit=0  restart=r");
